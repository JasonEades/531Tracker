...
**A modern, mobile-first workout tracker for Jim Wendler's 5/3/1 strength training program — with multi-user support, Google login, and a built-in Push/Pull/Legs program.**
...
- 📋 **First Set Last (FSL)** — optional 5×5 supplemental sets at the first main set's weight
- 5️⃣ **5s PRO mode** — all main sets capped at straight 5 reps (no AMRAP), the standard Leader cycle protocol
- 🧮 **Estimated 1RM** — auto-calculated after every AMRAP set using the Epley formula
...
- ⚖️ **Plate Calculator** — enter your available plates and bar weight; the app tells you exactly what to load
- 🛠️ **Equipment Setup** — configure your bar weight and plate inventory

### Push / Pull / Legs (PPL)
- 🗓️ **3-day or 6-day splits** — flexible schedule that fits any training week
- 💪 **Double progression** — automatically increments weight when you hit the top of your rep range
- 📈 **Estimated 1RM tracking** — compound lifts show e1RM after every session
- 🔗 **TM sync** — optionally push your best PPL e1RM back into your 5/3/1 training maxes
- 📖 **Session history** — full log of every completed PPL session with volume stats

### Multi-User & Auth
- 🔐 **Google Sign-In** — secure OAuth 2.0 login via Google
- 👤 **Per-user data isolation** — every user's lifts, cycles, equipment, and PPL programs are completely private
- 🆕 **First-login provisioning** — new users get default lifts and equipment automatically; existing data is claimed on first login
- 🛡️ **Admin panel** — enable/disable user accounts at `/admin/users` (accessible only to the configured admin)
...
│   └── Pages/
│       ├── Home.razor              # Dashboard (5/3/1 + PPL)
│       ├── WorkoutDetail.razor     # Active 5/3/1 session
...
│       ├── Login.razor             # Google sign-in page
│       ├── PplSetup.razor          # Create a PPL program
│       ├── PplSessionDetail.razor  # Active PPL session
│       ├── PplHistory.razor        # PPL session history
│       └── Admin/
│           └── UserManagement.razor  # Admin user enable/disable
├── Models/              # EF Core entity models (5/3/1 + PPL + Identity)
...
│   ├── PplProgramService       # PPL program creation & rotation
│   ├── PplSessionService       # PPL session management
│   ├── PplProgressionService   # PPL double progression & TM sync
│   ├── UserInitService         # First-login provisioning
│   ├── UserContextService      # Current user ID helper
│   └── AdminService            # User enable/disable
├── Data/
│   └── AppDbContext.cs  # EF Core SQLite context (extends IdentityDbContext)
...
3. Add an **Authorized redirect URI**: `https://localhost:PORT/signin-google`
4. Copy the Client ID and Secret into `appsettings.Development.json`
...
Then open your browser to the URL shown in the terminal and sign in with Google.

### First-Time Setup

1. **Sign in** with your Google account — default lifts and equipment are created automatically
2. **Create a Cycle** — tap **+ Create Cycle** on the dashboard and configure your options
3. *(Optional)* Enable **BBB**, **FSL**, **5s PRO**, or **Warmup Sets** (hover each toggle for an explanation)
4. **Configure Equipment** — set your bar weight and plate inventory for the plate calculator
5. **Start Training** — tap **▶ Start Workout** and log your sets
...
The configured `AdminEmail` in `appsettings.Development.json` is automatically granted the `Admin` role on startup. Admins can:

- View all registered users at `/admin/users`
- Enable or disable any non-admin account
- Disabled users are blocked at sign-in and cannot access the app

The admin nav link (🧑‍🤝‍🧑 **Users**) is only visible when signed in as an admin.
